...
def preprocess_sfm(input_matches_folder,out_folder, weight=0.01):
    ...
    #matches_json_path = "/root/autodl-tmp/SatelliteSfM/inputs/matches.json"
    
    if os.path.exists(input_matches_folder):
        print(f"[Preprocess SfM] Found matches.json at {input_matches_folder}")
        print("[Preprocess SfM] Importing matches to database...")
        # 调用导入脚本生成 database.db
        success = matches_importer.import_matches_json(out_folder, input_matches_folder)
        if not success:
            print("[Preprocess SfM] Error importing matches! Stopping.")
            return # 或者 raise Exception
    else:
        print("[Preprocess SfM] matches.json not found, falling back to standard COLMAP SIFT...")
